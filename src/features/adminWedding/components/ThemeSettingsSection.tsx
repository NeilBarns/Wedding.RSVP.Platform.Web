...
export function ThemeSettingsSection({ form }: { form: UseFormReturn<WeddingSettingsFormValues> }) {
  return <section className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-surface)] p-5 sm:p-7"><h2 className="flex items-center gap-2 font-[var(--font-display)] text-2xl"><Palette className="size-5" aria-hidden="true" />Theme settings</h2><p className="mt-1 text-sm text-[var(--color-muted)]">These values are configuration only for now; this page does not provide a live theme preview or load external fonts.</p><div className="mt-6 grid gap-5 sm:grid-cols-2"><ThemeField label="Theme key" name="theme.key" form={form} placeholder="editorial-linen" />{colors.map((item) => <ThemeField key={item.name} {...item} form={form} placeholder="#F5F1EA" />)}<ThemeField label="Heading font" name="theme.headingFont" form={form} /><ThemeField label="Body font" name="theme.bodyFont" form={form} /></div></section>
}
