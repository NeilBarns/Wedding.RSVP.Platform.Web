...
export function WeddingSettingsForm({ wedding, onSaved, onSessionExpired, onNotFound }: Props) {
  ...
  async function save(values: WeddingSettingsFormValues) {
    ...
    try {
      ...
      setSuccess(updated.status === 'archived' && oldStatus !== 'archived' ? 'Wedding archived.' : updated.status === 'published' && oldStatus !== 'published' ? 'Wedding published.' : updated.status === 'draft' && oldStatus === 'published' ? 'Wedding moved to draft.' : 'Wedding settings saved.')
    } catch (error) {
      setPending(null); setConfirmation(null)
      ...
      if (error instanceof ApiError && error.status === 422 && error.validationErrors) {
        for (const [path, messages] of Object.entries(error.validationErrors)) if (/^(partnerOneName|partnerTwoName|weddingDate|rsvpDeadline|dressCode|status|theme\.(key|primaryColor|secondaryColor|accentColor|backgroundColor|headingFont|bodyFont))$/.test(path)) form.setError(path as Parameters<typeof form.setError>[0], { type: 'server', message: messages[0] })
