...
export default function AdminWeddingPage() {
  ...
  return <section className="mx-auto max-w-5xl"><header className="mb-7 flex flex-wrap items-start justify-between gap-4"><div><p className="flex items-center gap-2 text-sm font-medium uppercase tracking-[0.12em] text-[var(--color-muted)]"><Settings2 className="size-4" aria-hidden="true" />Current wedding</p><h1 className="mt-2 font-[var(--font-display)] text-3xl sm:text-4xl">Wedding settings</h1><p className="mt-2 max-w-2xl text-[var(--color-muted)]">Manage core details, RSVP timing, guest-facing template, publication state, and theme configuration.</p></div><span className="rounded-full border border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-2 text-sm font-medium">{weddingStatusContent[wedding.status].label}</span></header><WeddingSettingsForm wedding={wedding} onSaved={setWedding} onSessionExpired={() => void refreshAuth(false).catch(() => undefined)} onNotFound={() => setNotFound(true)} /></section>
}
